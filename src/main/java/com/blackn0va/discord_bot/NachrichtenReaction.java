                    ...
                    if (ereignis.getChannel().getId().equals(Main.GPTChannelID)) {
                        ...
                        if (!ereignis.getMessage().getContentStripped().startsWith("!")) {
                            ...
                            if (answer.contains("You exceeded your current quota")) {
                                ...
                                if (answer.contains("Incorrect API key")) {
                                    ...
                                    ereignis.getChannel().sendTyping().queue();
                                    ereignis.getChannel().sendMessage(answer + ereignis.getAuthor().getAsMention())
                                            .queue();
                                }
                    ...
                    if (ereignis.getMember().getRoles()
                        ...
                        if (ereignis.getMessage().getContentStripped().startsWith("!say")) {
                            ...
                            ereignis.getJDA().getPresence().setActivity(
                                    Activity.streaming("stream",
                                            ereignis.getMessage().getContentStripped().substring(8)));
                            ...
                            Main.RegelnPostID = Main.bauplan.getTextChannelsByName("regeln", true).get(0).getHistory().retrievePast(100).complete().stream().filter(m -> m.getContentRaw().contains("§ 1")).findFirst().get().getId();
