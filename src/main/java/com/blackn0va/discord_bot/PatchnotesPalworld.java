...
public class PatchnotesPalworld {
    ...
    public static void CheckandSaveLink(String Version) throws IOException {
        ...
        if (!Main.PalworldVersionFile.equals(Main.PalworldPatchlink)) {
            ...
            writer.close();
            WriteLogs.writeLog("Neuer Palworld Patch wurde gefunden!");
            ...
            // eine Meldung aus
            WriteLogs.writeLog("Kein Neuer Palworld Patch gefunden!");
            GetPatchNotes();
        }
    ...
    public static void GetPalworldPatchnotes() {
        WriteLogs.writeLog("Prüfung auf neue Palworld Patches...");

        try {
            Main.PalworldPatchPages = null;

            Main.PalworldPatchPages = PatchnotesPalworld.getPages();
            WriteLogs.writeLog("Palworld Patchnotes Seiten: " + Main.PalworldPatchPages.size());
            ...
            if (!messages.isEmpty() && Main.PalworldPatchPages.size() > 0) {
                ...
                if (!eingebetteteInhalte.isEmpty()) {
                    ...
                    if (Main.PalworldCurrentPageNum <= PatchnotesPalworld.getPages().size()) {
                        ...
                        ersteWoerter = ersteWoerter.replaceAll("\\]", "");
                        WriteLogs.writeLog("Erste Wörter: " + ersteWoerter);
                        // prüfen ob text schon gespostet wurde
                        if (beschreibungEingebetteterInhalt.contains(ersteWoerter)) {
                            try {
                                WriteLogs.writeLog("Palworld Patch bereits gepostet");
                            } catch (Exception e) {
                                WriteLogs.writeLog("Fehler beim Bearbeiten der Nachricht: ");
                            }
                            ...
                            try {
                                ...
                                WriteLogs.writeLog("Palworld Patch wird gepostet");
                                ...
                                WriteLogs.writeLog("Fehler beim Senden der Nachricht: ");
                            }
            ...
            WriteLogs.writeLog("Fehler beim Abrufen der Patchnotes: ");
        }
    }
